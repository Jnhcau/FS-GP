...
class LassoSelector(BaseEstimator, SelectorMixin, TransformerMixin):
    ...
    def fit(self, X, y):
        self.model_ = LassoCV(cv=self.cv, random_state=self.random_state, max_iter=10000)
        self.model_.fit(X, y)
...
class RFSelector(BaseEstimator, SelectorMixin, TransformerMixin):
    ...
    def fit(self, X, y):
        ...
        self.model_.fit(X, y)
        self.support_ = self.model_.feature_importances_ > 0
        return self
...
class LGBMSelector(BaseEstimator, SelectorMixin, TransformerMixin):
    ...
    def fit(self, X, y):
        ...
        self.model_.fit(X, y)
        self.support_ = self.model_.feature_importances_ > 1e-5
        return self
